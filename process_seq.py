...
def samtools(sample, paths):
	"""
	Wrapper for applying samtools/bcftools/GATK for indel/SNP calling + conensus sequence generation.

	First converts SAM file to BAM format, then generates a reference dictionary, 
	and repairs readgroups in BAM file, which is required for GATK.
	...
	with open(paths["filtered_vcf_file"],"w") as f:
		run([BCFTOOLS, "filter", "-i",
			 f"FORMAT/DP>{VCF_DP} && (FORMAT/AD[*:1]/ FORMAT/DP)>{VCF_AF} && FORMAT/AD[*:1] != '*'","-Oz",
...
with open(STATS_OUTPUT_PATH, "w") as statsout:
	...
	# Extract keys (Metric names, e.g. number of snps) from first dictionary of final_stat list
	values_as_cols = [[*final_stats[0]]]
	# Extract values from each dict in final_stats list and append as elemnt to values_as_cols list
